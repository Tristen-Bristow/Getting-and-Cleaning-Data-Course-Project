...
dr <- "C:/Users/silly/Desktop/coursera/Coursera data intro R notes/coursera 3/UCI HAR Dataset/"
...
m_table5 <- aggregate(m_table4, by=list(t_av$activity.label),mean)
m_table6 <- aggregate(m_table4, by=list(t_av$subject),mean)
...
m_table5 <- m_table5[m_table5[!is.na(m_table5)]]
m_table6 <- m_table6[m_table6[!is.na(m_table6)]]

setwd(dr)
write.csv(m_table4, file = "mergedset.csv")
